...
def find_substring_tags(un_players, teams):
    i = 0
            ...
            if p in tag_p and len(tag_p) == per_team:
                if Utils.sanitize_uni(p.strip()).lower().startswith(Utils.sanitize_uni(comp_tag.strip()).lower()) \
                    and not Utils.sanitize_uni(p.strip()).lower().startswith(Utils.sanitize_uni(tag.strip()).lower()) \
                        and more_prefix(all_tags[comp_tag], comp_tag):
                    ...
                    and not Utils.sanitize_uni(p.strip()).lower().endswith(Utils.sanitize_uni(tag.strip()).lower()) \
                        and more_suffix(all_tags[comp_tag], comp_tag):
                    all_tags[tag].discard(p)
...
def assert_correct(teams, un_players, per_team, num_teams):
    ...
    if (found_num_teams==num_teams or (found_num_teams<num_teams and len(un_players)>=(per_team*num_teams)))\
...
def select_tops(all_tags, per_team, num_teams, teams, players):
    ...
    for _ in range(num_teams):
        ...
        if len(all_tags)==0:
            break
        all_tags = dict(sorted(all_tags.items(), key=lambda item: (len(item[1]), count_pre_tags(item[0],item[1])), reverse=True))
...
def clean_tags(teams):
    for ind, tag in enumerate(list(teams.keys())):
        if list(teams.keys()).count(tag)>1:
            temp = tag
            a = 1
            while temp in teams:
                temp = f'{tag}-{a}'
                a+=1
            teams[temp] = teams.pop(list(teams.keys())[ind])
        else:
            dup_indx = tag.rfind('-')
            if tag[:dup_indx] not in teams and tag[dup_indx+1:].isnumeric() and dup_indx>=int(len(tag)/2):
                teams[tag[:dup_indx]] = teams.pop(list(teams.keys())[ind])
...
def tag_algo(players, per_team, num_teams):
    ...
    select_tops(all_tag_matches, per_team, num_teams, teams, players) #select best tags that meet requirements (sort by number with prefixes, and actual tags for ones with too many)
    
    #print(teams)
    assert_correct(teams, players, per_team, num_teams) #backup in case of rare errors
    clean_tags(teams)
