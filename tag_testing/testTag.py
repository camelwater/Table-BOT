    ...
    def copy_solution(tags_possibilities:Dict[Tuple[str, str, int], Set[Tuple[str,str]]]) -> Dict[Tuple[str, str, int], Set[Tuple[str,str]]]:
        ...
        for tag, players in tags_possibilities.items():
            ...
            for fc, player in players:
                players_copy.add( (copy.copy(fc), copy.copy(player)))
            solution_copy[( copy.copy(tag[0]), copy.copy(tag[1]), copy.copy(tag[2]) )] = players_copy
...
def getTagsSmart(fc_players:List[Tuple[str, str]], playersPerTeam:int) -> Dict[Tuple[str, str], Tuple[str, str]]:
    ...
    __clean(tag_counts)
    #print("Time this took:", time.time() - startTime)
...
if __name__ == "__main__":
    players = [('1', "x#1"), ('2', "xxx"), ('3', "o3Ryan@x"), ('4', "¢unt"), ('5', "cool kid"), 
            ('6', "coolio"), ('7', "GG EZ"), ('8', "gassed up"), ('9', "gas mob"), ('10', "b hello"), ('11', "you hello"), ('12', "sup HELLO ")]
    # players = {'pringle@MV':0,'5headMV':0,'hello LTA':0,'LTAX':0,
    #     'jaja LTA':0,'stupid@LTA':0,'poop MV':0,'MVMVMVMV':0,'LTA Valpo':0,"mom's MVin":0}
    
    #players2 = []
    # for i,p in enumerate(players):
    #     players2.append((str(i), p))
    tick = time.time()
    print(getTagsSmart(players, 2))
