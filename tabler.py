...
class Table():
    ...
    def change_tag(self,player, tag, reundo=False):
        try:
            #p_indx = player
        ...
        #if not reundo:
            #self.modifications.append([('?changetag', p_indx,old_tag, tag)])
        return ret
        ...
        try:
            if '-' in score or '+' in score:
                ...
                if not reundo:
                    ...
                    try:
                        if self.warnings[int(gp)*4-3][0] != "Table has been manually modified by the tabler for this GP.":
                            self.warnings[int(gp)*4-3].insert(0, "Table has been manually modified by the tabler for this GP.")
                    except:
                        self.warnings[int(gp)*4-3] = ["Table has been manually modified by the tabler for this GP."]
    ...
    def unpenalty(self, player, unpen, reundo=False):
        ...
        if player not in self.pens:
            ...
            if unpen ==None:
                ...
                if not reundo:
                    self.modifications.append([('?unpen total', orig_pen)])
    ...
    def edit_dc_status(self,L, reundo=False): 
         ...
         if not reundo: self.modifications.append(mods)
         return ret
    ...
    def change_room_size(self, l, self_call = False, reundo=False): #TODO (unfinished) -temporarily resolved (?undo should completely resolve this)
        ...
        if not reundo and not self_call: self.modifications.append(mods)
        return ret
    ...
    def edit_race(self, l, reundo=False):
        ...
        for num, elem in enumerate(l):
            ...
            ret+='{} race {} placement changed to {}{}.'.format(p_indx, raceNum, correct_pos+1, '\n' if num==len(l)-1 else "")
            mods.append(('?editrace', raceNum, orig_pos, correct_pos))
    ...
    def undo(self, j):
        if '?edit change' == j[0] or '?edit add'==j[0]:
            ...
            self.edit_race([[j[1], j[3], j[2]]], reundo=True)
    ...
    def redo(self, j):
        if '?edit' == j[0] or "?edit add" == j[0]:
            ...
            self.edit_race([[j[1], j[2], j[3]]], reundo=True)
            
        #elif 'edittag' in j[0]:
            #self.edit_tag_name([[j[1], j[2]]], reundo=True)
        
        #elif 'changetag' in j[0]:
            #self.change_tag(j[1], j[3], reundo=True)
    ...
    def undo_commands(self, num): #TODO
        if num == 0: #undo all
    ...
    def redo_commands(self, num):
        if num == 0: #redo all
            if len(self.undos)>0:
                ...
                self.modifications = reversed(self.undos)
                self.undos = []
