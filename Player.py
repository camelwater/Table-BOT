...
class Player:
    ...
    subs: List[Tuple] = field(default_factory=list, hash=False, repr=False) #players this player has subbed in for (indx 0 = player, 1 = races played)
    races_played: int = field(default=None, hash=False, repr=False)
    ...
    def get_full_name(self, num_races=0) -> str:
        ...
        for sub in self.subs:
            for rec_sub in sub[0].subs:
                sub_races+=rec_sub[1]
            sub_races+=sub[1]
            full_name.append(f"{sub[0].get_full_name(-1)}({sub[1]})")
        
        full_name.append(f"{self.name}{self.figure_out_num_races(num_races, sub_races)}")
        return '/'.join(full_name)
    
    def figure_out_num_races(self, num_races, sub_races):
        if num_races == -1: #don't include num_races in full_name
            return ""
        this_races = (num_races-sub_races) if self.races_played is None else self.races_played
        return f"({this_races})"
    ...
    def combine_scores(self, out_player):
    ...
    def combine_pens(self, out_player):
        self.pens+=out_player.getPens()
